...
void parse_rules(char *in_fn, vector<range_point>* vec){
    FILE        *fp;
    ...
    while( fscanf(fp, "%s %d\n", pre_exp, &portnum) != EOF ){
        ...
        insert_rules(vec, prefix, prelen, portnum);
    }
    //printf("done with the insertion");
    fclose(fp);
}
                 ...
                 const u_char *pktdata)
{
    ...
    verdict = ip_lookup(&vec, dst_addr);
...
int main(int argc, char **argv)
{
    ...
    //num_line.reserve(200020);
    parse_rules(argv[2], &vec);
    refine_rule(&vec);
