...
// struct BtNode                   *bt_pre_line;       /* pointer to the pre_line node of the binary tree */
// struct Prpoint                  pre_line;
vector<P_point>                	num_line;
...
void parse_rules(char *in_fn, vector<P_point>* pre_line){
    FILE        *fp;
    ...
    while(temp <= 32) {
        ...
        while( fscanf(fp, "%s %d\n", pre_exp, &portnum) != EOF ){
            ...
            if(prelen == temp){
                cout << "insert" << prelen << endl;
                insert_rule(pre_line, prefix, prelen, portnum);
            }
        }
        fclose(fp);
        temp++;
                 ...
                 const u_char *pktdata)
{
    ...
    verdict = lookup_ip(&num_line, dst_addr);
...
int main(int argc, char **argv)
{
    ...
    /* build binary trie */
    // bt_pre_line = init_btnode();
    num_line.reserve(200000);
    parse_rules(argv[2], &num_line);
